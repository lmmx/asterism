...
To enable debug logging during development, set the `ASTERISM_DEBUG` environment variable:

```sh
ASTERISM_DEBUG=1 cargo run
```

or with the installed binary

```
ASTERISM_DEBUG=1 asterism
```

This will write detailed logs to `/tmp/asterism_debug.log` showing:
